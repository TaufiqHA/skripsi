			                            ...
			                            <td class="px-4 py-3 flex items-center justify-end">
			                                ...
			                                <div id="dropdown{{ $item->id }}" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
			                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown{{ $item->id }}-button">
			                                        <li>
			                                            <a data-modal-target="default-modal{{ $item->id }}" data-modal-toggle="default-modal{{ $item->id }}" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</a>
			                                        </li>
			                                    </ul>
			                                </div>
			                                <!-- modal edit start -->
											...
											<div id="default-modal{{ $item->id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
											    <div class="relative p-4 w-full max-w-2xl max-h-full">
											        ...
											        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
											            ...
											            <!-- Modal body -->
											            <form action=" {{ route('admin.sempro.save') }} " method="post" class="p-4 md:p-5 grid grid-cols-6 gap-5" id="saveSemproForm" enctype="multipart/form-data">
											            	@csrf
												    ...
												    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
												        ...
												        <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
												            ...
												            <div class="flex justify-center items-center space-x-4">
												                ...
												                <button id="saveSemproButton" type="submit" class="py-2 px-3 text-sm font-medium text-center text-white rounded-lg bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:focus:ring-primary-900">
													                Continue
													            </button>
												            </div>
												        </div>
...
@section('script')
	<script>
		const saveSemproButton = document.getElementById('saveSemproButton')
		const saveSemproForm = document.getElementById('saveSemproForm')

		saveSemproButton.addEventListener('click', () => {
			saveSemproForm.submit()
		})
	</script>
@endsection